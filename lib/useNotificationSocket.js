...
export function useNotificationSocket(enabled = true) {
  ...
  useEffect(() => {
    if (enabled && activeStore?.id) fetchNotifications();
  }, [enabled, fetchNotifications, activeStore?.id]);
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      ws.onclose = (event) => {
        ...
        if (event.code === 4001) {
          // Token expired or invalid, trigger an API call to refresh it before reconnecting
  ...
  }, [activeStore?.id, addNotification, tokenReady, fetchNotifications]);
}
