...
api.interceptors.request.use(
  (config) => {
    if (typeof window !== 'undefined') {
      ...
      // Send active store ID so backend can resolve the correct tenant
      try {
        const stored = JSON.parse(localStorage.getItem('store-selection') || '{}');
        const storeId = stored?.state?.activeStore?.id;
        if (storeId) {
...
export const orderAPI = {
  ...
  customerByEmail: (email) => api.get('/orders/customers/by-email/', { params: { email } }),
};
